function locateLatestCoordinate(currCoordinate, coordinateRecords) {
  let tempRecord = coordinateRecords;
  let locationIndex = tempRecord.findIndex(
    (record) => record.x === currCoordinate.x && record.y === currCoordinate.y
  );
  ...
  if (locationIndex === -1) {
    ...
    tempRecord[locationIndex].present = tempRecord[locationIndex].present + 1;
  }
  return tempRecord;
}

function q1(puzzleInput) {
  ...
  splittedPuzzleInput.forEach((direction) => {
    if (direction === "^") {
      currCoordinate.y = currCoordinate.y + 1;
    }

    if (direction === ">") {
      currCoordinate.x = currCoordinate.x + 1;
    }
    ...
    coordinateRecords = locateLatestCoordinate(
      currCoordinate,
      coordinateRecords
    );
  });

  console.log("finalRecord: ", coordinateRecords.length);
}

//=================================================================================

async function q2(puzzleInput) {
  console.log("q2");
  const splittedPuzzleInput = puzzleInput.split("");
  let coordinateRecords = [
    {
      x: 0,
      y: 0,
      present: 2, //santa & robo-santa starting point
    },
  ];

  let currCoordinateSanta = {
    x: 0,
    y: 0,
  };

  let currCoordinateRoboSanta = {
    x: 0,
    y: 0,
  };

  splittedPuzzleInput.forEach((direction, index) => {
    if (direction === "^" && (index + 1) % 2 === 0) {
      currCoordinateRoboSanta.y = currCoordinateRoboSanta.y + 1;
    } else if (direction === "^") {
      currCoordinateSanta.y = currCoordinateSanta.y + 1;
    }

    if (direction === ">" && (index + 1) % 2 === 0) {
      currCoordinateRoboSanta.x = currCoordinateRoboSanta.x + 1;
    } else if (direction === ">") {
      currCoordinateSanta.x = currCoordinateSanta.x + 1;
    }

    if (direction === "v" && (index + 1) % 2 === 0) {
      currCoordinateRoboSanta.y = currCoordinateRoboSanta.y - 1;
    } else if (direction === "v") {
      currCoordinateSanta.y = currCoordinateSanta.y - 1;
    }
